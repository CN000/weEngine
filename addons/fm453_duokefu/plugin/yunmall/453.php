...
//	$openid = $_GPC['openid']; (OPENID由客服助手模块主程序处理，此页自动继承)
	...
	$sqlnum = 'SELECT COUNT(*) FROM ' . tablename('yunmall_order') . $condition;
	$params = array();
	...
	if ($total > 0) {
		...
		$sql = 'SELECT * FROM ' . tablename('yunmall_order') . $condition . $limit;
		...
		foreach ($list as &$value) {
			...
			$value['dispatch'] = pdo_fetchcolumn("SELECT `dispatchname` FROM " . tablename('yunmall_dispatch') . " WHERE id = :id", array(':id' => $value['dispatch']));
			if ($s < 1) {
			...
			$value['goods'] = pdo_fetchall("SELECT g.*, o.total,g.type,o.optionname,o.optionid,o.price as orderprice FROM " . tablename('yunmall_order_goods') . " o left join " . tablename('yunmall_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid", array(':orderid' => $value['id']));
			$item['goods'] = $value['goods'];
		}
		$return['yunmall']['orders']=$list;
		$return['yunmall']['pagename']="云商城订单";
	}	